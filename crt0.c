...
unsigned SWSC_SIZE = 32;//32 * 1024; // 32k total size (spill zone) for stack cache
//******************************************************************************
...
void _start() 
{
  ...
  _addr_base_ext = (unsigned)((int) brk) + SWSC_SIZE; // swsc ext base
  _addr_base_spm = 0x20;//0x800; // small (2k) SPM stack cache from 0x800 down
